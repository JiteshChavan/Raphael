...
class MambaInnerFnNoOutProjCond(torch.autograd.Function):
    ...
    def forward(
        ...
        x, z = xz.chunk(2, dim=1)
        conv1d_bias = conv1d_bias.contiguous() if conv1d_bias is not None else None
        conv1d_out = causal_conv1d_cuda.causal_conv1d_fwd_cond(x, conv1d_weight, conv1d_bias, True, init_states)
        # We're being very careful here about the layout, to avoid extra transposes.
        # We want delta to have d as the slowest moving dimension
        # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
        x_dbl = F.linear(rearrange(conv1d_out, "b d l -> (b l) d"), x_proj_weight)  # (bl d)
...
class MambaInnerFnCond(torch.autograd.Function):
...
class BiMambaInnerFn(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        dconv1d_out_f_b, ddelta_f_b, dA_b, dB_f_b, dC_f_b, dD_b, ddelta_bias_b, dz_b, out_z_b = (
...
def mamba_inner_fn_no_out_proj_cond(
    xz,
    conv1d_weight,
    conv1d_bias,
    x_proj_weight,
    ...
    return MambaInnerFnNoOutProjCond.apply(
        xz,
        conv1d_weight,
        conv1d_bias,
        x_proj_weight,
